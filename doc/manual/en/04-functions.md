...
| `pgmoneta_ext_fips()`       |   Default      | None   | Return `true` if the PostgreSQL backend's OpenSSL is running in FIPS mode, `false` otherwise. <br>Uses `EVP_default_properties_is_fips_enabled()` on OpenSSL 3.0+ and `FIPS_mode()` on OpenSSL 1.1.x.|
